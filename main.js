  ...
  Turbo.on('group-participants-update', async (anu) => {
      ...
      if (anu.action == 'add') {
	    ...
	    capt = welc.replace('@user', tag).replace('@name', username).replace('@bio', about).replace('@date', tanggal).replace('@desc', descrip).replace('@group', mdata.subject);
	      Turbo.send2ButtonLoc(mdata.id, buff, capt, 'Agregame a FB\nhttps://www.facebook.com/VeinTiiUno', '⦙☰ MENU', '/menu', '⏍ INFO GP', '/infogp', false, {
	      contextInfo: {  
        ...
        Turbo.sendButtonLoc(mdata.id, buff, capt, 'Agregame a FB\nhttps://www.facebook.com/VeinTiiUno', '👋🏻', 'unde', false, {
	      contextInfo: { 
            mentionedJid: Turbo.parseMention(capt)
	      } 
	    });
